...
namespace NETCORE
{
    public class Snake
    {
        ...
        public Snake(int initicalX, int initicalY, ConsoleColor headColor, ConsoleColor bodyColor, int bodyLength = 3)
        {
            _headColor = headColor;
            _bodyColor = bodyColor;

            Head = new Pixel(initicalX, initicalY, _headColor);

            for (int index = bodyLength; index >= 0; index--)
            {
                Body.Enqueue(new Pixel(Head.X - index - 1, initicalY, _bodyColor));
            }
            Draw();
        }
